...
### This script applies a few changes to the second branch script based on Tobias Egner's paper.
...
for(i in 1:length(datafiles_list)){
  ...
  if (nrow(incong_errorDat) >= outlier_num){ #for when the participant has outlier_num or more than outlier_num incongruent errors
    ...
    # Below is exclusionary criterion change based on Egner's paper. I exclude subjects who have gen_hit below 55%!
    if (gen_hit > 0.5){
