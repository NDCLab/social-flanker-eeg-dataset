...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Test_exps2\\Test_exps\\EF_try\\EF_TRY\\inhib_night2_lastrun.py',
...
for thisPrac_BlockLoop in prac_BlockLoop:
    ...
    for thisPrac_trialLoop in prac_trialLoop:
        ...
        for thisPrac_cueLoop in prac_cueLoop:
            ...
            #2
            pulse_started_01 = False
            pulse_ended_01 = False
            ...
            while continueRoutine:
                ...
                #3
                if cuePresented_2.status == STARTED and not pulse_started_01:
                    if whichCue == 'SHAPE':
                        port.write([0x01])#S1 for 'SHAPE' cue. 
                        time.sleep(PulseWidth)
                        pulse_start_time = globalClock.getTime()
                        pulse_started_01 = True
                    elif whichCue == 'COLOR':
                        port.write([0x02])#S2 for 'COLOR' cue. 
                        time.sleep(PulseWidth)
                        pulse_start_time = globalClock.getTime()
                        pulse_started_01 = True
                if pulse_started_01 and not pulse_ended_01:
                    if globalClock.getTime() - pulse_start_time >= 0.005:
                        #port.write([0x02])
                        pulse_ended_01 = True
        ...
        for thisPrac_stimLoop in prac_stimLoop:
            ...
            while continueRoutine:
                ...
                #3
                if centerPresented_2.status == STARTED and not pulse_started:#
                    if ethnicity == 1 and rotCenter == 0:
                        port.write([0x17]) #S23
                        #time.sleep(PulseWidth)
                        pulse_started = True
                        pulse_start_time = globalClock.getTime()
                    elif ethnicity == 1 and rotCenter == 1:#
                        port.write([0x1B])#S27
                        #time.sleep(PulseWidth)
                        pulse_started = True
                        pulse_start_time = globalClock.getTime()
                        ...
                        pulse_started = True
                        pulse_start_time = globalClock.getTime()
                    elif ethnicity == 2 and rotCenter == 1:#
                        port.write([0x25])#S37
                        #time.sleep(PulseWidth)
                        pulse_started = True
                        pulse_start_time = globalClock.getTime()
                        ...
                        pulse_started = True
                        pulse_start_time = globalClock.getTime()
                    elif ethnicity == 3 and rotCenter == 1:#
                        port.write([0x2F])#S47
                        #time.sleep(PulseWidth)
                        pulse_started = True
                        pulse_start_time = globalClock.getTime()
                        ...
                        pulse_started = True
                        pulse_start_time = globalClock.getTime()
                    elif ethnicity == 4 and rotCenter == 1:#
                        port.write([0x39])#S57
                        #time.sleep(PulseWidth)
                        pulse_started = True
                        pulse_start_time = globalClock.getTime()
                ...
                if keyResp_2.status == STARTED and not waitOnFlip:
                    theseKeys = keyResp_2.getKeys(keyList=['z', 'm'], waitRelease=False)
