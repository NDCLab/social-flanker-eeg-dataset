...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\NDClab\\Desktop\\social-context-eeg\\Multi_EF_with_ALL_triggers_faster_gb\\Multi_EF_with_ALL_triggers\\multi-ef-o_s1_r2_e1-EEG-Inhib-Task_faster_changeRsi_lastrun.py',
...
# Initialize components for Routine "rsiRoutine"
rsiRoutineClock = core.Clock()
reminderDur = 500
keys_counted_late = []
respTime = 0
feedOn = False
...
rightReminder_text_8 = visual.TextStim(win=win, name='rightReminder_text_8',
    ...
    languageStyle='LTR',
    depth=-4.0);
keyResp_late = keyboard.Keyboard()
...
for thisBlockLoop in blockLoop:
    ...
    for thisTrialLoop in trialLoop:
        ...
        # ------Prepare to start Routine "rsiRoutine"-------
        continueRoutine = True
        # update component parameters for each repeat
        # pick the ISI for the next routine
        # this code component is set to 'both' because we need to remove the 'np'
        ...
        respTime = 0
        feedOn = False
        feedback.setText(feedText)
        LeftReminder_text_8.setText(rightReminder)
        rightReminder_text_8.setText(rightReminder)
        keyResp_late.keys = []
        keyResp_late.rt = []
        _keyResp_late_allKeys = []
        # keep track of which components have finished
        rsiRoutineComponents = [feedback, LeftReminder_text_8, rightReminder_text_8, keyResp_late]
        for thisComponent in rsiRoutineComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        t = 0
        _timeToFirstFrame = win.getFutureFlipTime(clock="now")
        rsiRoutineClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
        frameN = -1
        
        # -------Run Routine "rsiRoutine"-------
        while continueRoutine:
            # get current time
            t = rsiRoutineClock.getTime()
            tThisFlip = win.getFutureFlipTime(clock=rsiRoutineClock)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
            ...
            # *feedback* updates
            if feedback.status == NOT_STARTED and feedOn:
                # keep track of start time/frame for later
            ...
            if feedback.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > feedback.tStartRefresh + .3-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if LeftReminder_text_8.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > LeftReminder_text_8.tStartRefresh + thisRSI-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if rightReminder_text_8.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rightReminder_text_8.tStartRefresh + thisRSI-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # *keyResp_late* updates
            waitOnFlip = False
            if keyResp_late.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                keyResp_late.frameNStart = frameN  # exact frame index
                keyResp_late.tStart = t  # local t and not account for scr refresh
                keyResp_late.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(keyResp_late, 'tStartRefresh')  # time at next scr refresh
                keyResp_late.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(keyResp_late.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(keyResp_late.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if keyResp_late.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > keyResp_late.tStartRefresh + thisRSI-frameTolerance:
                    # keep track of stop time/frame for later
                    keyResp_late.tStop = t  # not accounting for scr refresh
                    keyResp_late.frameNStop = frameN  # exact frame index
                    win.timeOnFlip(keyResp_late, 'tStopRefresh')  # time at next scr refresh
                    keyResp_late.status = FINISHED
            if keyResp_late.status == STARTED and not waitOnFlip:
                theseKeys = keyResp_late.getKeys(keyList=['1', '8'], waitRelease=False)
                _keyResp_late_allKeys.extend(theseKeys)
                if len(_keyResp_late_allKeys):
                    keyResp_late.keys = _keyResp_late_allKeys[-1].name  # just the last key pressed
                    keyResp_late.rt = _keyResp_late_allKeys[-1].rt
        ...
        trialLoop.addData('feedback.started', feedback.tStartRefresh)
        trialLoop.addData('feedback.stopped', feedback.tStopRefresh)
        trialLoop.addData('LeftReminder_text_8.started', LeftReminder_text_8.tStartRefresh)
        trialLoop.addData('LeftReminder_text_8.stopped', LeftReminder_text_8.tStopRefresh)
        trialLoop.addData('rightReminder_text_8.started', rightReminder_text_8.tStartRefresh)
        trialLoop.addData('rightReminder_text_8.stopped', rightReminder_text_8.tStopRefresh)
        # check responses
        if keyResp_late.keys in ['', [], None]:  # No response was made
            keyResp_late.keys = None
        trialLoop.addData('keyResp_late.keys',keyResp_late.keys)
        if keyResp_late.keys != None:  # we had a response
            trialLoop.addData('keyResp_late.rt', keyResp_late.rt)
        trialLoop.addData('keyResp_late.started', keyResp_late.tStartRefresh)
        trialLoop.addData('keyResp_late.stopped', keyResp_late.tStopRefresh)
        # the Routine "rsiRoutine" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
